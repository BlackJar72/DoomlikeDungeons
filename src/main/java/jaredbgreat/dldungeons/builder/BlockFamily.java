...
public class BlockFamily implements IBlockPlacer {
	...
	public static BlockFamily makeBlockFamily(String json) {
		...
		if(data.has("name")) {
			...
			throw new RuntimeException(Info.LOG_NAME + " tried to load BlockFamily with no Name!");
		}
		if(data.has("blocks")) {
			...
			throw new RuntimeException(Info.LOG_NAME + " tried to load BlockFamily with no blocks!");			
		}
		if(blocks.isEmpty()) {
			throw new RuntimeException(Info.LOG_NAME + " tried to load BlockFamily empty block list!");			
		}		
