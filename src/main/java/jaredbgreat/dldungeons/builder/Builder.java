...
/**
 * This class will likely be cannibalized into components for 
 * subclasses of Structure, StructureStart and the pieces of 
 * Structures -- or maybe not, could possibly still be re-instated
...
//		if (MinecraftForge.EVENT_BUS.post(new DLDEvent.PlaceDungeonBegin(random, chunkX, chunkZ, world))) return;
...
//		dungeon.preFinalize();
//		dungeon = null;
//		MinecraftForge.EVENT_BUS.post(new DLDEvent.PlaceDungeonFinish(random, chunkX, chunkZ, world, dungeon));
...
//						ChunkGenerator chunkGenerator, AbstractChunkProvider chunkProvider) throws Throwable {	
//		if(world.isRemote) return; // Do not perform world-gen on the client!
//		if (MinecraftForge.EVENT_BUS.post(new DLDEvent.PlaceDungeonBegin(random, chunkX, chunkZ, world))) return;
//		DoomlikeDungeons.profiler.startTask("Create Dungeons");
//		Dungeon dungeon = new Dungeon(random, 
...
//								.anyMatch(id -> id == world.getDimension().getType().getId()))) {
