...
public class Room extends AbstractRoom {
	...
	public Room(int beginX, int endX, int beginZ, int endZ, int floorY, int ceilY, 
		...
		for(int i = beginX + 1; i < endX; i++)
			for(int j = beginZ + 1; j < endZ; j++) {
				if(dungeon.map.getRoom(i, j) == 0) dungeon.map.setRoom(id, i, j);
				dungeon.map.setCeilY((byte)ceilY, i, j);
				dungeon.map.setNCeilY((byte)ceilY, i, j);
				dungeon.map.setFloorY((byte)floorY, i, j);
				dungeon.map.setNFloorY((byte)floorY, i, j);
				if(!sky) dungeon.map.setCeiling(cielingBlock, i, j);
				dungeon.map.setFloor(floorBlock, i, j);
				dungeon.map.setWall(wallBlock1, i, j);	
				dungeon.map.setHasLiquid(false, i, j);	
				dungeon.map.setIsWall(false, i, j);					
			}
	...
	private void assignEdge(Dungeon dungeon, int x, int z) {
		if((dungeon.map.getRoom(x, z) == 0) 
				|| (dungeon.rooms.get(dungeon.map.getRoom(x, z)).sky && !sky)
				|| (isSubroom)) {
			dungeon.map.setRoom(id, x, z);
			if(!sky) dungeon.map.setCeiling(cielingBlock, x, z);
			dungeon.map.setFloor(floorBlock, x, z);
			dungeon.map.setWall(wallBlock1, x, z);	
			dungeon.map.setHasLiquid(false, x, z);
			dungeon.map.setIsWall(!sky, x, z);
			dungeon.map.setIsFence(fenced, x, z);
		}
		if(dungeon.map.getCeilY(x, z) < (byte)ceilY) 
			dungeon.map.setCeilY((byte)ceilY, x, z);
		if(dungeon.map.getNCeilY(x, z) < (byte)ceilY) 
			dungeon.map.setNCeilY((byte)ceilY, x, z);
		if(dungeon.map.getFloorY(x, z) < (byte)floorY) 
			dungeon.map.setFloorY((byte)floorY, x, z);
		if((dungeon.map.getNFloorY(x, z) > (byte)nFloorY) 
				|| (dungeon.map.getNFloorY(x, z) == 0)) 
			dungeon.map.setNFloorY((byte)floorY, x, z);				
	}
	...
	protected void addSpawners(Dungeon dungeon) {
		...
		if(!hasEntrance && (ConfigHandler.difficulty.multimob(dungeon.random) || isNode)) {
			...
			for(int i = 0; i < num; i++) {
				...
				if(dungeon.random.nextInt(4) == 0) y = dungeon.map.getCeilY(x, z);
				else y = dungeon.map.getFloorY(x, z);
			...
			if(dungeon.random.nextInt(4) == 0) y = dungeon.map.getFloorY(x, z);
			else y = dungeon.map.getCeilY(x, z);
			int lev = levAdjust(ConfigHandler.difficulty.moblevel(dungeon.random), dungeon);
			if(lev >= 0) {
				mob = dungeon.theme.allMobs[lev].get(dungeon.random.nextInt(dungeon.theme.allMobs[lev].size()));
				Spawner s = new Spawner(x, y, z, id, lev, mob);
		...
		if(isNode && !hasEntrance) {
			x = (int)realX;
			z = (int)realZ;
			if(dungeon.map.getHasLiquid(x, z)) {
				y = dungeon.map.getCeilY(x, z);
			}
			else {
				y = dungeon.map.getFloorY(x, z) - 1;
			} 
	...
	protected void addDoor(Dungeon dungeon, int x, int z, boolean xOriented) {
		...
		dungeon.map.setIsDoor(true, x, z);
	}
	...
	public Room connector(Dungeon dungeon, int dir, int xdim, int zdim, int height, Route source) {
		...
		if(dungeon.map.getRoom(x + xSeedDir, z + zSeedDir) != 0) {
				return dungeon.rooms.get(dungeon.map.getRoom(x + xSeedDir, z + zSeedDir));
			}
	...
	private void walkway(Dungeon dungeon) {
		...
		for(int i = beginX; i <= endX; i++) 
			for(int j = beginZ; j <= endZ; j++) {
				dungeon.map.setFloorY((byte)(dungeon.map.getFloorY(i, j) - drop), i, j);
				dungeon.map.setHasLiquid(true, i, j);
			}
	...
	private void cutin(Dungeon dungeon) {
		...
		for(int i = beginX; i <= endX; i++) 
			for(int j = beginZ; j <= endZ; j++) {
				dungeon.map.setIsWall(true, i, j);
			}
